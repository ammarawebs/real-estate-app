...
const UpdateListing = () => {
  ...
  useEffect(() => {
    const fetchListing = async () => {
      ...
      listingRef.current = listing._id
      setFormData(listing);
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(`/listing/${listingRef.current}`);
    } catch (error) {
          ...
          <div className="flex flex-col w-full sm:w-2/4 gap-4 my-9 justify-start items-start">
            ...
            <div className="flex w-full sm:max-w-lg gap-3 ">
              <input
                ...
                accept="image/*"
                multiple
                className=" border border-gray-300 p-4 w-2/3 rounded-md"
              />
              <button
                type="button"
                onClick={handleImageSubmit}
                className=" bg-green-600 cursor-pointer hover:bg-green-700 text-white p-4 w-1/3 rounded-md"
              >
            ...
            {formData.imageUrls.length > 0 &&
              formData.imageUrls.map((imageUrl, index) => {
                return (
                  <div
                    className="flex flex-row justify-between items-center gap-3 w-full sm:max-w-lg border border-gray-300 p-7 rounded-lg"
                    key={index}
                  >
                    <img
                      src={imageUrl}
                    ...
                    <button
                      ...
                      className=" text-red-500 font-semibold cursor-pointer border border-red-500 uppercase h-14 w-28 rounded-md hover:bg-red-600 hover:text-white "
                    >
                      Delete
                    </button>
                  </div>
            ...
            <button
              ...
              className=" text-center w-full sm:max-w-lg bg-slate-700 hover:bg-slate-800 text-white p-3 rounded-lg disabled:opacity-50"
            >
